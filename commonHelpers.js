...
</li>`).join("")}async function b(r,o,n){const i="42523874-92f67aed3babfb2c9b36364a5",e="https://pixabay.com/api/",t=r.trim(),c=await fetch(`${e}?key=${i}&q=${t}&image_type=photo&orientation=horizontal&safesearch=true&page=${o}&per_page=${n}`);if(!c.ok)throw new Error(c.status);return c.json()}const u=document.querySelector(".gallery"),m=document.querySelector(".input-img"),$=document.querySelector(".form");document.querySelector(".btn-sub");const f=document.querySelector(".loading"),a=document.querySelector('[data-action="load-more"]'),p=document.querySelector(".spinner");f.style.display="none";a.style.display="none";function q(r){r.preventDefault(),u.innerHTML=null;const o=m.value.trim();if(!o)return S();f.style.display="inline-block",a.style.display="inline-block",b(o,1,15).then(n=>{const i=n.totalHits||0;if(l*15>i)return a.classList.add("is-hidden"),v();l=1,L=o;const t=n.hits;if(t.length)u.innerHTML=h(t),new y(".gallery a.gallery-link",{captionsData:"alt",captionDelay:300}).refresh();else return u.innerHTML="",w()}).catch(n=>console.log(n)).finally(()=>{f.style.display="none"}),m.value=""}let l=1,L;$.addEventListener("submit",q);a.addEventListener("click",r=>{p.classList.remove("is-hidden"),l++,b(L,l,15).then(o=>{h(images),new y(".gallery a.gallery-link",{captionsData:"alt",captionDelay:300}).refresh(),p.scrollIntoView,p.classList.add("is-hidden")}).catch(o=>alert(o.message))});function w(){g.error({message:"Sorry, there are no images matching your search query. Please try again!",title:"Error",titleColor:"#fff",backgroundColor:"rgba(239, 64, 64, 1)",messageColor:"#fff",position:"topRight"})}function S(){g.warning({title:"Увага!",position:"topRight",backgroundColor:"orange",message:"Поле пошуку не може бути порожнім!"})}function v(){g.info({title:"Info",position:"topCenter",backgroundColor:"blue",message:"Упс! Більше немає зображень..."})}
