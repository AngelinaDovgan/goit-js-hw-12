...
</li>`).join("")}async function b(r,o,n){const i="42523874-92f67aed3babfb2c9b36364a5",e="https://pixabay.com/api/",t=r.trim(),c=await fetch(`${e}?key=${i}&q=${t}&image_type=photo&orientation=horizontal&safesearch=true&page=${o}&per_page=${n}`);if(!c.ok)throw new Error(c.status);return c.json()}const a=document.querySelector(".gallery"),m=document.querySelector(".input-img"),$=document.querySelector(".form");document.querySelector(".btn-sub");const p=document.querySelector(".loading"),l=document.querySelector('[data-action="load-more"]'),u=document.querySelector(".spinner");let f=1,L;p.style.display="none";l.style.display="none";function q(r){r.preventDefault(),a.innerHTML=null;const o=m.value.trim();if(!o)return S();p.style.display="inline-block",l.style.display="inline-block",b(o,1,15).then(n=>{const i=n.totalHits||0;if(f*15>i)return l.classList.add("is-hidden"),v();L=o;const t=n.hits;if(t.length)a.innerHTML=h(t),new y(".gallery a.gallery-link",{captionsData:"alt",captionDelay:300}).refresh();else return a.innerHTML="",w()}).catch(n=>console.log(n)).finally(()=>{p.style.display="none"}),m.value=""}$.addEventListener("submit",q);l.addEventListener("click",r=>{u.classList.remove("is-hidden"),f+=1,b(L,f,15).then(o=>{const n=o.hits;a.innerHTML=h(n),new y(".gallery a.gallery-link",{captionsData:"alt",captionDelay:300}).refresh(),u.scrollIntoView,u.classList.add("is-hidden")}).catch(o=>alert(o.message))});function w(){g.error({message:"Sorry, there are no images matching your search query. Please try again!",title:"Error",titleColor:"#fff",backgroundColor:"rgba(239, 64, 64, 1)",messageColor:"#fff",position:"topRight"})}function S(){g.warning({title:"Увага!",position:"topRight",backgroundColor:"orange",message:"Поле пошуку не може бути порожнім!"})}function v(){g.info({title:"Info",position:"topCenter",backgroundColor:"blue",message:"Упс! Більше немає зображень..."})}
